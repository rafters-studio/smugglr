...
- **An edit confined to a hash-excluded column is no longer dropped, via a new `converge_columns` class.** Excluding a column from the content hash means a change confined to that column produces a hash MATCH -- and a hash match was the diff's skip condition, so the row was classified `identical` and never transferred, even with a newer `updated_at`. Silent data loss with no error, no conflict, and no bucket anyone inspects. The fix distinguishes two things `exclude_columns` was conflating: `exclude_columns` stays "out of the hash, and off the wire **on the directional push/pull path**" (derived or huge values, embeddings being the motivating case), while the new `[sync].converge_columns` means "out of the hash but still synced".
  ...
  **Two limits on `converge_columns` in this release specifically.** It is **native-only**: the npm/wasm package builds its sync config through a separate binding that exposes no `convergeColumns` field, so JS consumers cannot set it at all this release (the exclude/converge overlap refusal *is* wired on that path, so the feature cannot be half-enabled there). And it is **inert on a table whose `timestamp_column` resolves for no row** -- reconciliation is by timestamp, so with no ordering signal the rows classify as `identical` and nothing propagates, with no warning that the feature is doing nothing. Both tracked as #323.
