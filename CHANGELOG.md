...
- **`pull` with `exclude_columns` no longer nulls the excluded column in the local database.** `transfer_rows` strips every `[sync].exclude_columns` match from each row before it is sent, and on `pull` the destination is the LOCAL database -- but the native apply derived its column list from the *destination's* `PRAGMA table_info` rather than from the incoming row, and bound every column the row did not carry as an explicit NULL. So a pull overwrote the local value of the one column an operator configured never to leave the machine, on every row that transferred, under every write arm that writes. Silent by construction: an excluded column is out of the content hash, so nothing downstream compares it and no test asserting "the sync succeeded" can see it. Shipped in 0.4.x. The applied column set is now the incoming row's keys intersected with the destination's columns -- primary-key columns always included, since they are the conflict target -- and a row that omits a column emits `INSERT INTO ... ON CONFLICT(pk) DO UPDATE SET <carried non-PK columns>`, so an unmentioned column keeps its stored value. All three write arms are covered, including the `ordering_unavailable` fallback that degrades `newer_wins` to a blind overwrite on a table carrying none of its ordering columns; that arm is exactly where the protection would have gone missing unnoticed. Rows in one batch carrying different key sets are grouped by key set and written under separate statements -- a stripping sender is uniform, so that is one group in practice, but nothing in the trait promises it. (#324, #322 Part A)

  **Two risks, and why no existing deployment pays them.** Giving up `INSERT OR REPLACE` on the subset path costs two behaviors: `INSERT OR REPLACE` resolves a conflict on *any* uniqueness constraint while `ON CONFLICT(pk)` resolves only the primary key, and `INSERT OR REPLACE` fires DELETE triggers and `ON DELETE CASCADE` where `DO UPDATE` does not. The new shape is therefore confined to rows that actually omit a column: a row carrying every destination column emits SQL byte-identical to before, so a deployment with no `exclude_columns` configured is unchanged and pays neither. The shape is not new to this codebase either -- `local_wins` and `newer_wins` already resolve through `ON CONFLICT(pk)`.

  **Scope, stated rather than implied.** This fixes the native apply path: the directional `pull`, `stash`/`retrieve` (whose destination is a `LocalDb`), and the multicast apply. It does **not** make `exclude_columns` safe everywhere. A directional `push` to a remote still destroys the column at the destination -- the http-sql, D1, and wasm adapters derive their columns from `rows[0].keys()` and emit `INSERT OR REPLACE`, which SQLite executes as DELETE+INSERT, resetting an unmentioned column to its schema default. `exclude_columns` is also still not applied at all on the `stash`/`retrieve` and multicast `Want` paths, so a column excluded to keep it off the wire still leaves the machine there (#322 Parts B and C). And a column that is `NOT NULL` with no `DEFAULT`, omitted from a row that does not yet exist at the destination, still fails the insert -- as before, with an error naming the table and the column. `DataSource::upsert_rows` and the plugin protocol doc now state what an absent column means, which is the gap that let three implementations each answer it differently.
...
- **A keyless node no longer guesses whether a datagram is plaintext.** `maybe_decrypt`'s no-key arm decided "is this plaintext JSON?" by testing the first byte for `{` or `[`. Encrypted datagrams begin with a 24-byte random nonce, so that test matched 2 byte values in 256: 1 foreign datagram in 128 was classified as plaintext, and the unit test pinning the invariant failed at the same rate (measured 6 failures in 1000 runs, on every platform -- it reddened a macOS job on #312's CI). The arm now classifies nothing and hands the datagram to the `Msg` parser, which rejects a foreign cluster's ciphertext as unparseable. No packet's *fate* changes -- nothing was accepted before that is rejected now, or the reverse, with one narrow exception: a plaintext datagram with leading whitespace before its JSON was dropped by the old first-byte test and is now parsed and applied, which is correct, since `serde_json` skips leading whitespace and the old test did not. What does change is the reported *reason*: a keyless node now reports `IgnoreReason::Malformed` for foreign ciphertext where it previously reported `Undecryptable` for roughly 127 packets in 128, so `Undecryptable` is no longer reachable at all without a configured key. An embedder counting that variant as a wrong-key signal will see it go to zero in keyless deployments. Relatedly, the old per-packet `warn!` on that path is gone; a keyless node now warns **once at bind** instead, which fires whether or not any foreign traffic ever arrives -- so an operator who meant to set `[broadcast].secret` and did not is told immediately rather than never. The `multicast` module doc is corrected alongside: its cluster-isolation claim ("a foreign key's datagrams fail to decrypt and are dropped") describes AEAD authentication and therefore requires a key. A node running without one decrypts nothing and verifies nothing, so it has no isolation in the security sense and no integrity guarantee on what it accepts -- any host on the subnet can originate rows for it. Run with a key. No wire change; `PROTOCOL_VERSION` is unaffected. (#313)
